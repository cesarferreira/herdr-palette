...
export const theme = { background: "#29284f", panel: "#3c3b68", text: "#e9e8ff", muted: "#a7a4df", accent: "#ffe11a" };

export interface PaletteDeps { run: (item: PaletteItem) => Promise<CommandResult>; close: () => void }

/** Rows the chrome always owns: heading, input, the blank line below it, footer, bottom padding. */
const CHROME_ROWS = 5;
...
export function mountPalette(renderer: CliRenderer, allItems: PaletteItem[], deps: PaletteDeps) {
  ...
  function redraw() {
    ...
    panel = new BoxRenderable(renderer, { id: "palette", flexDirection: "column", width: "100%", height: "100%", backgroundColor: theme.background, paddingLeft: 2, paddingRight: 2, paddingBottom: 1 });
    ...
    if (status) panel.add(new TextRenderable(renderer, { id: "status", content: status.replace(/\s+/g, " ").slice(0, Math.max(20, renderer.width - 4)), fg: theme.accent }));
    panel.add(new TextRenderable(renderer, { id: "footer", content: `enter select   ↑/↓ move   ${items.length} commands`, fg: theme.muted }));
